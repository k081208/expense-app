...
| 会社 | 対応 | 根拠 |
| --- | --- | --- |
| 楽天カード / JACCS / PayPay カード / アメリカン・エキスプレス / JCB | 正式対応 | 実メールで送信元・件名の形を確認済み。本文の形は実環境での確認結果に合わせて調整する。JCB の案内メールには請求金額が載らないため、JCB は支払日のみ（金額は Gmail から得られない） |
...
| 失敗時のヒント | 金額・支払日・カードを読み取れなかったメールについて、「金額らしき行」「日付らしき行」「カード番号らしき行」を数字をマスクした形で最大 8 行ずつ表示する。実メールのラベルが想定と違うときに規則を直す材料にする |
| 未対応の会社も試す | チェックすると、エディオン（オリコ）・イオン・セゾンを探索で観測した送信元・件名だけで少数取得し、本文の形を確認できる。結果は採用候補に入らず、正式対応の根拠にもしない。au PAY は実メール待ちのため試行もしない |
